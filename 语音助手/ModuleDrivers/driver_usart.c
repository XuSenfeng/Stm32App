...
static platform_mutex_t uart2_recv_mutex;
/*
...
{
    HAL_NVIC_SetPriority(USART2_IRQn, 8, 1);    // 设置USART3中断的优先级
...
int USART2_Read(char *c, int timeout)
{
	...
	while(ret){
		...
		if(ring_buffer_read((unsigned char *)c, &uart2_buffer)==0)
		{
			return ret;
		}else{
			ret = platform_mutex_lock_timeout(&uart2_recv_mutex, timeout);
		}
	}
	return ret;
}
void UART3_Lock_Init()
{
	...
	platform_mutex_init(&uart2_recv_mutex);
	platform_mutex_lock(&uart2_recv_mutex);
}
...
void USART2_IRQHandler(void)
{
    ...
    if (((isrflags & USART_SR_RXNE) != RESET))
    {
	  ...
	  while((huart1.Instance->SR & USART_SR_TXE)==0);
	  huart1.Instance->DR = c;
	  platform_mutex_unlock_FromISR(&uart2_recv_mutex);
    }
