...
static void topic1_handler(void* client, message_data_t* msg)
{
    ...
    MQTT_LOG_I("-----------------------------------------------------------------------------------");
	if(strstr((char*)msg->message->payload, "LED1"))
	{
...
void UART2RetParser(void const * argument)
{
	...
	while(1)
	{
		...
		//获取WIFI模块发送来的命令
		HAL_USRT2_Secv(&buf[i], (int)portMAX_DELAY);
		//进行解析
		/*1. 什么时候解析 */
		if(i && buf[i-1] == '\r' && buf[i] == '\n')
		{
			...
			/*2. 怎么解析 */
			buf[i+1] = 0;
			if(strstr(buf, "LED1_H") )
			{
				//接收到成功标志
				printf("OK1\r\n");
				HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_RESET);
				
			}else if( strstr(buf, "LED1_L"))
			{
				printf("OK2\r\n");
				HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_SET);
			}else if( strstr(buf, "LED2_H"))
			{
				printf("OK3\r\n");
				HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_RESET);
			}else if( strstr(buf, "LED2_L"))
			{
				printf("OK4\r\n");
